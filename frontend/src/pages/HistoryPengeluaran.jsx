...
const HistoryPengeluaran = () => {
  const [historyPengeluaran, setHistoryPengeluaran] = useState([]);

  const fetchDataHistory = () => {
    getAllTransaksi().then(({ historyPengeluaran }) => {
      setHistoryPengeluaran(historyPengeluaran);
    });
  };
  ...
  console.log(historyPengeluaran);

  return (
    <div className="flex flex-col p-5">
      <Card>
        {historyPengeluaran.length === 0 ? (
          <DataEmpty
          ...
          <table className="w-full">
            ...
            <tbody className="border-b border-[#E1E2E9]">
              {historyPengeluaran.length > 0 &&
                historyPengeluaran.map((history) => (
                  <tr
                    key={history._id}
